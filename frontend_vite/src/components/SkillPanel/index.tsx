...
export function SkillPanel({ sessionId, onNavigateToConfig, isActive = false }: SkillPanelProps) {
  ...
  const handleImportServerPath = useCallback(async () => {
  ...
  const handleFileSelected = useCallback(async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showMessage("success", t('skills.messages.imported', { name: data.skill?.name || f.name }));
      await fetchSkills();
      if (data.skill?.name) {
        await fetchSkillDetail(data.skill.name);
      }
        ...
        <div className="card flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex items-center">
            ...
            <button
              ...
              title={t('skills.importUploadHint')}
            >
            ...
            >
              {t('skills.actions.importServerPath')}
            </button>
            <input
              type="file"
              accept=".md,.zip"
