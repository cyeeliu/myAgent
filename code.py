...
Need: pip install openai python-dotenv pyyaml + .env with OPENAI_API_KEY
      (optional: OPENAI_BASE_URL, MODEL_ID, FALLBACK_MODEL_ID)
...
import ast, json, os, subprocess, time, random, threading, re, queue
...
# The rest of the agent speaks in Anthropic-style content blocks (text /
# tool_use) and tool_result user messages. This adapter is the only place
# that knows the OpenAI wire format, so swapping providers later only touches
# this section — everything downstream (compaction, transcripts, teammates)
# keeps working against the block shape.
...
# ── Event Sink / Session / Permission ──

# The agent loop no longer prints or inputs directly. It emits typed events
# through a sink and asks a Permission object. CLI gets a TerminalSink +
# CliPermission (behavior identical to the old print/input code); the API
# gateway gets ChannelSink(s) + FuturePermission (resolved by WS/REST frames).
...
class TerminalSink(EventSink):
    ...
    def emit(self, kind: str, payload: dict):
        ...
        # token/error/compacted/done: CLI has no live rendering for these.
...
# Back-compat alias for any external code importing the old name.
...
# permission_hook is invoked directly by agent_loop (it needs the session's
# Permission object + event sink); log_hook stays in the registry.
...
# ── Memory System ──
# Persistent, cross-session knowledge. Mirrors s09_memory: each memory is one
# Markdown file under .memory/ with YAML frontmatter; MEMORY.md is a one-line-
# per-memory index rebuilt from those files. Three operations compose:
#   load_memories()      — select relevant memories (LLM) and return their content
#                          for injection into the turn (called once per user turn)
#   extract_memories()   — after the turn ends, pull new user/feedback/project
#                          facts out of the dialogue and write them as files
#   consolidate_memories() — when the file count grows, merge/dedupe via LLM
# The index is always present in the system prompt (cheap); relevant *content*
# is injected per turn. Files live under workdir() so each session/workspace
# has its own .memory/.
...
# Per-conversation state now lives in Session; no module-level globals here.
...
def agent_loop(session: Session):
    ...
    while True:
        ...
        if not has_tool_use(response.content):
            ...
            # Memory: extract new facts from the full (append-only) record and
            # consolidate when the catalog grows. Best-effort — never let memory
            # failures break the turn's clean exit.
            try:
                written = extract_memories(session.record)
                if written:
